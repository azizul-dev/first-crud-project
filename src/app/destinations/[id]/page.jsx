...
import { MapPin, Star, Users, Calendar, Languages, CheckCircle2, ShoppingCart, Heart } from "lucide-react";
    ...
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      <div className="relative w-full h-[360px] rounded-3xl overflow-hidden mb-8 shadow-2xl">
        ...
        <div className="absolute bottom-5 left-6 right-6">
          ...
          <p className="text-white/70 text-sm">{duration} · Tropical Experience</p>
        </div>
      </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-3 gap-3 mt-6">
              {[
                { icon: <Users size={18} />, value: groupSize, label: "Group size" },
                { icon: <Calendar size={18} />, value: bestTime, label: "Best time" },
                { icon: <Languages size={18} />, value: language, label: "Language" },
              ].map((item, i) => (
                <div
                  key={i}
                  className="bg-neutral-50 dark:bg-neutral-800 rounded-xl p-4 text-center"
                >
                  <div className="flex justify-center mb-2 text-emerald-600">{item.icon}</div>
                  <span className="block text-sm font-medium text-neutral-800 dark:text-neutral-200">
                    {item.value}
                  </span>
                  <span className="text-xs text-neutral-400 mt-0.5 block">{item.label}</span>
                </div>
              ))}
            </div>
          </div>
